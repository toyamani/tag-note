...
function Widget() {
  console.log("keyof typeof TAG");
  const [tag, setTag] = useSyncedState<(typeof TAG)[keyof typeof TAG]>(
    "tag",
    TAG.SPECS
  );
  const [showName, setShowName] = useSyncedState<boolean>("showName", true);
  // console.log("tag", tag, Object(TAG));
